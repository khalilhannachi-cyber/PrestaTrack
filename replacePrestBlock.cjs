...
const newGrid = `<div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
              {/* Total */}
...
let startIndex = c.indexOf('<div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">');
let endIndex = c.indexOf('          {/* Message si aucun dossier */}');

if (startIndex !== -1 && endIndex !== -1) {
  // we have to adjust endIndex back to exactly the closing div of the grid.
  // We can just replace everything from startIndex to endIndex - length of `          )}\n\n`
  ...
  // Actually, we just need to replace between startIndex and `            </div>\n          )}\n\n          {/* Message`
  const endGridIndex = c.indexOf('            </div>\n          )}', startIndex);
  
  if (endGridIndex !== -1) {
    c = c.substring(0, startIndex) + newGrid + c.substring(endGridIndex + 18);
    // wait, endGridIndex + 18 is `            </div>`. Let's just use replace using the exact string.
  }
  
  // Wait, let's do a reliable replacement using a script:
}

// Strip remaining visual emojis just in case
c = c.replace(/[💼📊💰⚙️👤✅❌⚠️🚪🔄🚀👋💡📅🔍📝✏️🗑️🏠👥🏢📈➕📥🗂️💳💸📉👔🗃️📌🎯🔒📋📂💾💵📃]/gu, '');
