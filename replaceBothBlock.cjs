...
const doReplace = (file, newGrid, classToMatch) => {
  let c = fs.readFileSync(file, 'utf8');

  let startIndex = c.indexOf('<div className="' + classToMatch + '">');
  if (startIndex === -1) { console.log('Not found in ' + file); return; }

  // Use regex to find the end of the block
  // We look for the ending that matches `</div>` followed by `)}`
  const match = c.substring(startIndex).match(/<\/div>\s*\)\}/);
  if (match) {
    const endGridIndex = startIndex + match.index + 6; // index of </div> + 6 = after </div>
    c = c.substring(0, startIndex) + newGrid + '\n' + c.substring(endGridIndex);
    fs.writeFileSync(file, c);
    console.log('Success for ' + file);
  } else {
    console.log('End not found for ' + file);
  }
};

const newGridPrest = `<div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
              {/* Total */}
...
const newGridFin = `<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              {/* Total */}
              <div className="bg-white rounded-xl border border-comar-neutral-border p-4">
                <div className="flex items-center justify-between">
                  <div>
...
doReplace('src/pages/prestation/PrestationDashboard.jsx', newGridPrest, 'grid grid-cols-1 md:grid-cols-5 gap-4 mb-6');
doReplace('src/pages/Finance/FinanceDashboard.jsx', newGridFin, 'grid grid-cols-1 md:grid-cols-4 gap-4 mb-6');
